...
void cv_hrono::wait(unique_lock<mutex>& lock) 
{
	...
	while (!pair->fulfilled) pair->cv.wait(lock);

	// Semanticka provera - da li je zaista cv koja je izasla iz cekanja ona prva is reda (front), ukoliko
	// slucajno iz nekog razloga nije prepusta se procesor drugoj niti (yield) i nakon toga se ponovo proverava
...
{
	// Funkcionise veoma slicno kao metoda wait, sa tim sto ako se desi timeout
	cv_pair* pair = new cv_pair;							
